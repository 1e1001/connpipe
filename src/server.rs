...
async fn run_server<T: Router>(router: &T) -> CResult<()> {
	...
	let config = router.get_config().await;
	if addresses.len() == 0 {
	...
	enum TaskMessage {
		...
		NeedResolving(
			usize,
			CResult<(TcpStream, Address, Option<String>, [u8; 16], u8)>,
		),
				...
				TaskMessage::Accept(v) => {
					...
					pinned_tasks.push(task::spawn(async move {
						let result = async {
							...
							let (subport, end) =
								if total == 16 && u128::from_ne_bytes(magic_buf) == MAGIC {
									...
									for i in 0..MAX_LEN_LEN {
										if subport_len > config.max_subport_len {
											trace!("[thread {}] subport len exceeded max_subport_len, dropping connection", id);
											Err(CError::EarlyBreak)?;
										}
									...
									match String::from_utf8(subport_buf) {
										Ok(v) => (Some(v), 0),
										Err(e) => {
											trace!("[thread {}] utf-8 conversion error: {}", id, e);
											(None, total)
										}
									}
								} else {
							...
							Ok((stream, address, subport, magic_buf, end.try_into().unwrap()))
						}
						.await;
						match result {
							Err(CError::EarlyBreak) => TaskMessage::ConnectionComplete(id, Ok(())),
							result => TaskMessage::NeedResolving(id, result),
						}
					}));
				}
				TaskMessage::NeedResolving(id, Ok((stream, address, subport, buf, end))) => {
						...
						Some(resolved) => {
							...
							pinned_tasks.push(task::spawn(async move {
								TaskMessage::ConnectionComplete(
									id,
									async move {
										trace!("[thread {}] connecting", id);
										let outwards =
											TcpStream::connect((resolved.addr, resolved.port))
												.await?;
										...
										if let Some(subport) = resolved.subport {
											...
											while subport_len > 0 {
												let shift =
													(subport_len & 0x7F).try_into().unwrap();
												subport_len >>= 7;
												let shift = if subport_len > 0 {
													shift | 0x80
												} else {
													shift
												};
										...
										loop {
											match tokio::select! {
												n = outwards_read.read(&mut outwards_buf) => {
													outwards_write.write_all(&outwards_buf[0..n?]).await
												},
												n = inwards_read.read(&mut inwards_buf) => {
													inwards_write.write_all(&inwards_buf[0..n?]).await
												}
											} {
												Ok(_) => {}
												Err(e) => match e.kind() {
